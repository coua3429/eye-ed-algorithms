// Eye Emergency Clinical Decision Support - JavaScript (Updated v2.0)
...
class EyeEmergencyApp {
    ...
    // Data Management - Updated to include all 9 algorithms
    async loadAlgorithms() {
        try {
            ...
            for (const file of algorithmFiles) {
                try {
                    const response = await fetch(file + '?v=' + Date.now()); // Cache busting
                    if (response.ok) {
            ...
            if (this.algorithms.length === 0) {
                this.showToast('No algorithms could be loaded. Please check your connection.', 'error');
            }
        } catch (error) {
    ...
    // ESR Report Generation - Enhanced for better mobile compatibility
    copyESRReport() {
...
${triageLog}`;
    ...
    // Fallback copy method for older browsers/mobile devices
    fallbackCopyToClipboard(text) {
        ...
        try {
            ...
            this.showToast('Copy not supported - please select and copy manually', 'error');
        } finally {
...
// Service Worker Registration for PWA - Updated version
